...
import (
	...
	"github.com/gkwa/enoughparse/internal/logger"
)
...
func init() {
	...
	rootCmd.PersistentFlags().StringVar(&cfgFile, "config", "", "config file (default is $HOME/.enoughparse.yaml)")
	rootCmd.PersistentFlags().CountVarP(&verbosity, "verbose", "v", "increase verbosity level (can be used multiple times)")
...
func initConfig() {
	if cfgFile != "" {
		...
		viper.SetConfigName(".enoughparse")
	}
